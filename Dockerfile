...
RUN apt-get update && apt-get install -y bash curl \
    ...
    && rm -rf /var/lib/apt/lists/* \
    && mkdir -p /.infisical /infisical-keyring \
    && chmod 777 /.infisical /infisical-keyring
...
RUN python3.11 -m venv /var/lib/tinymotion_env \
    ...
    && cp /opt/tinymotion_backend/scripts/tinymotion-backend-infisical /usr/local/bin/tinymotion-backend-infisical \
    && chmod +x /usr/local/bin/tinymotion-backend-infisical \
